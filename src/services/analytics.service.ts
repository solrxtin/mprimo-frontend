...
class AnalyticsService {
  ...
  async getAnalytics(
    ...
    switch (timeframe) {
      case "daily":
        defaultStart = new Date(end.getTime() - 1 * 24 * 60 * 60 * 1000); // Last 1 day
        break;
      case "weekly":
        defaultStart = new Date(end.getTime() - 7 * 24 * 60 * 60 * 1000); // Last 7 days
        break;
      case "monthly":
        defaultStart = new Date(end.getTime() - 30 * 24 * 60 * 60 * 1000); // Last 30 days
        break;
    }
  ...
  async getDashboardAnalytics(
        ...
        $project: {
          ...
          conversionRate: {
            $cond: [
              ...
              {
                $multiply: [
                  { $divide: ["$totalPurchases", "$totalViews"] },
                  100,
                ],
              },
              0,
    ...
    // Get summary metrics
    const todayMetrics = await this.getSummaryMetrics(
      entityType,
      today,
      new Date()
    );
    const weekMetrics = await this.getSummaryMetrics(
      entityType,
      startOfWeek,
      new Date()
    );
    const monthMetrics = await this.getSummaryMetrics(
      entityType,
      startOfMonth,
      new Date()
    );
