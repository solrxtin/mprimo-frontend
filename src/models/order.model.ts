...
const orderSchema = new mongoose.Schema<Order>({
    ...
    payment: {
        ...
        amount: {
            ...
            validate: {
                validator: function(this: Order, value: number) {
                    if (this.items) {
    ...
    shipping: {
        ...
        carrier: {
            ...
            required: [
                function(this: Order) {
                    return ['shipped', 'delivered', 'returned'].includes(this.shipping?.status || '');
                },
        ...
        estimatedDelivery: {
            type: Date,
            validate: {
                validator: function(this: Order, v: Date) {
...
orderSchema.pre('save', function(next) {
    ...
    const previousStatus = this.isModified('status') ? this.get('status') : this.status;

    if (previousStatus === 'cancelled' && this.status !== 'cancelled') {
        this.invalidate('status', 'Cannot change status from cancelled');
    }

    if (currentStatusIndex > -1 && statusOrder.indexOf(previousStatus) > currentStatusIndex) {
        this.invalidate('status', `Cannot change status from ${previousStatus} to ${this.status}`);
    }
