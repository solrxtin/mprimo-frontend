...
const calculateCartSummary = (items: CartItem[]): CartSummary => {
  const subtotal = items.reduce((total, item) => {
    const price = item.selectedVariant?.price || item?.product.inventory.listing?.instant?.salePrice || 0;
...
export const useCartStore = create<CartState>((set, get) => ({
  ...
  addToCart: (product, quantity = 1, selectedVariant) => {
    ...
    const existingItemIndex = items.findIndex(item => {
      const existingVariantKey = item.selectedVariant
        ...
        : undefined;
      return generateCartItemKey(item.product?._id ?? "", existingVariantKey) === itemKey;
    });
  ...
  updateQuantity: (productId, quantity, variantKey) => {
    ...
    const updatedItems = items.map(item => {
      ...
      if (generateCartItemKey(item.product._id ?? "", existingVariantKey) === itemKey) {
