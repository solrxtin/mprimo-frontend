...
const LogoutModal = ({ isOpen, onClose, isLoading, logout }: Card) => {
  return (
    <Modal2 isOpen={isOpen} onClose={isLoading ? () => {} : onClose}>
      <div className="inline-block overflow-hidden text-left pb-4  px-3 md:px-6 lg:px-7 relative align-bottom transition-all transform bg-[white] rounded-[24px] shadow-xl sm:my-8 sm:align-middle sm:max-w-[587px] sm:w-full">
        <div className="py-4 md:py-8">
          <div className="flex items-center justify-between">
            ...
            <button onClick={onClose} disabled={isLoading} className="disabled:opacity-50 disabled:cursor-not-allowed">
              <X />
            </button>
          </div>
          ...
          <div className="space-y-4 md:space-y-6">
            ...
            <Image
              src={Assets.logout}
              alt="Logout Image"
              className="h-[120px] md:h-[220px] mx-auto"
            />
            ...
            <div className="flex flex-col space-y-2 mt-5">
              <FullButton
                action={logout}
                disabled={isLoading}
                name={isLoading ? "Logging out..." : "Yes Please"}
                color="blue"
              />
              <Button
                variant="ghost"
