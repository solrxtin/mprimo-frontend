...
export const cartService = {
  ...
  async updateCartItem(productId: string, data: UpdateCartRequest): Promise<CartResponse> {
    const response = await fetchWithAuth(`${BASE_URL}/cart/update-cart/${productId}`, {
      method: 'PUT',
      body: JSON.stringify(data)
    });
    return response.json();
  ...
  async clearCart(): Promise<CartResponse> {
    const response = await fetchWithAuth(`${BASE_URL}/cart/clear`, {
      method: 'DELETE'
    });
    return response.json();
  },
  ...
  async getCartSummary(): Promise<{ success: boolean; data: { itemsCount: number; totalValue: number } }> {
    const response = await fetchWithAuth(`${BASE_URL}/cart/cart-summary`);
    return response.json();
  },
